...
# Every container in the hermes-agent-epgg compose project.
mapfile -t CONTAINERS < <(docker ps -a --filter "name=hermes-agent-epgg-" --format '{{.Names}}')
[ "${#CONTAINERS[@]}" -eq 0 ] && { say "no hermes-agent-epgg containers found"; exit 0; }

tg_alert() {
  local msg="$1"
  local tok; tok=$(docker exec hermes-agent-epgg-postgres-1 psql -U boss -d boss_ir -t -A -c "SELECT value FROM runtime_config WHERE key='TELEGRAM_BOT_TOKEN'" 2>/dev/null | tr -d '[:space:]')
...
for c in "${CONTAINERS[@]}"; do
  ...
  docker restart "$c" >>"$LOG" 2>&1 || (cd "$R" && docker compose up -d >>"$LOG" 2>&1)
  if [ "$n" -ge 4 ]; then
