...
const serviceBase = () => (process.env.UNIPILE_SERVICE_URL || 'http://bos_unipile:8000').replace(/\/$/, '');
...
function actionFromOverview(body: unknown) {
  ...
  return {
    instructions: [
      ...
      'Do not approve, queue, or publish. Save revised drafts for human review in BOS.',
    ],
...
function linkedinGptOpenApi() {
  ...
  return {
    openapi: '3.1.0',
    info: {
      title: 'BOS LinkedIn Agent GPT Bridge',
      version: '1.0.0',
      description: 'Read LinkedIn context and save reviewed LinkedIn post drafts for human approval in BOS.',
    },
    servers: [{ url: server }],
    components: {
      securitySchemes: {
    ...
    paths: {
      ...
      '/api/linkedin-system/gpt/draft': {
        post: {
          ...
          summary: 'Save a LinkedIn draft for human approval in BOS. Does not publish.',
          security: [{ GptActionKey: [] }],
          requestBody: {
            required: true,
            content: {
