...
export function computeDailyMetrics(records: RecordRow[]): DailyMetric[] {
  ...
  push('blood_glucose_mgdl', firstNum(latest(records, 'BloodGlucose')?.payload, ['mgdl', 'mg_per_dl', 'mg_per_dL']));
  push('body_temp_c', firstNum(latest(records, 'BodyTemperature')?.payload, ['celsius', 'c']));
  push('basal_body_temp_c', firstNum(latest(records, 'BasalBodyTemperature')?.payload, ['celsius', 'c']));
  push('skin_temp_c', firstNum(latest(records, 'SkinTemperature')?.payload, ['celsius', 'c']));
  push('weight_kg', firstNum(latest(records, 'Weight')?.payload, ['kg']));
  push('height_cm', firstNum(latest(records, 'Height')?.payload, ['cm']));
  push('body_fat_pct', firstNum(latest(records, 'BodyFat')?.payload, ['pct', 'percent']));
  push('body_water_kg', firstNum(latest(records, 'BodyWaterMass')?.payload, ['kg']));
  push('bone_mass_kg', firstNum(latest(records, 'BoneMass')?.payload, ['kg']));
  push('lean_mass_kg', firstNum(latest(records, 'LeanBodyMass')?.payload, ['kg']));
  push('bmr_kcal', firstNum(latest(records, 'BasalMetabolicRate')?.payload, ['kcal_per_day', 'kcal']));
