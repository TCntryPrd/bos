...
import type { DeviceRow, IngestBody, IngestResult, WireRecord } from './types.js';
import { INGEST_BATCH_LIMIT, RECORD_TYPES } from './types.js';
...
import {
  ...
  findDeviceByTokenHash, insertDevice, insertPairingCode, lastSyncAt, loadThresholdConfig,
  lockUserDayForRollup, recordsForDay, recordsRange, replaceDailyMetrics, setDeviceToken,
  softDeleteRecord, touchDeviceLastSeen, upsertRecord, upsertSyncState,
} from './repo.js';
...
export async function overview(pool: pg.Pool, tenantId: string) {
  ...
  const [paired, last, rows] = await Promise.all([
