...
/** PRIMARY voice: self-hosted OmniVoice Studio (ships with the product, $0/call, CPU). */
async function omniVoiceTts(text: string, voice?: VoiceOpts): Promise<Buffer> {
  const fd = new FormData();
  fd.append('text', text);
  fd.append('effect_preset', voice?.effect_preset ?? 'broadcast');
  if (voice?.instruct) fd.append('instruct', voice.instruct);
  if (voice?.profile_id) fd.append('profile_id', voice.profile_id);
  if (voice?.speed) fd.append('speed', String(voice.speed));
  const res = await fetch(`${OMNIVOICE_URL}/generate`, {
    // OmniVoice gates non-/generate admin routes on a loopback Origin; send a loopback
    // value so the request is accepted regardless of the bridge-gateway source IP.
    method: 'POST', body: fd, headers: { Origin: 'http://localhost:3900' },
    signal: AbortSignal.timeout(150_000),
  });
  if (!res.ok) { const body = await res.text().catch(() => ''); throw new Error(`omnivoice ${res.status}: ${body.slice(0, 220)}`); }
  if (!(res.headers.get('content-type') || '').includes('audio')) throw new Error('omnivoice non-audio');
  return Buffer.from(await res.arrayBuffer());
}

/** FALLBACK voice: Gemini TTS (fast, used only if OmniVoice is unavailable). */
...
export interface VoiceOpts { instruct?: string; profile_id?: string; effect_preset?: string; speed?: number; geminiVoice?: string }

/** Advisor voice — OmniVoice (ships) first, Gemini fallback. Returns a playable WAV buffer. */
export async function generateVoice(text: string, voice?: VoiceOpts): Promise<{ wav: Buffer; engine: string }> {
  try {
    ...
    console.warn('[board] OmniVoice TTS failed, falling back to Gemini:', (err as Error).message);
...
const ADVISOR_VOICES = ['Charon', 'Kore', 'Puck', 'Fenrir', 'Aoede', 'Zephyr', 'Orus', 'Leda'];

/** Generate an anime portrait (PNG) for an advisor via Gemini image, from an appearance blurb. */
async function generatePortrait(appearance: string): Promise<Buffer | null> {
  const key = process.env.GEMINI_API_KEY;
  if (!key) return null;
  const prompt = `Professional anime-style portrait illustration, head and shoulders, ${appearance}, calm confident expression, clean softly-lit deep-navy studio gradient background, polished modern anime art style, corporate executive headshot.`;
  try {
 ...
 *  name, title, bio, persona, voice, and an anime portrait. */
export async function generateAdvisor(tenantId: string, opts: { position: string; model_label?: string; seat_index?: number }): Promise<{ id: string; display_name: string }> {
  ...
  const prompt = `Design a distinctive advisory-board member for this authority/role: "${opts.position}".\n` +
    ...
    `APPEARANCE: <short portrait description — gender, age range, hair, attire — for an anime headshot>\n` +
  ...
  const png = await generatePortrait(appearance);
...
export async function updateAdvisor(tenantId: string, id: string, fields: { display_name?: string; title?: string; seat_index?: number; model_label?: string; system_addendum?: string }): Promise<void> {
  const t = currentTenantId(tenantId);
  const pool = getPool();
