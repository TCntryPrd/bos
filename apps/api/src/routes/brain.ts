        ...
        while (iterationCount < MAX_TOOL_ITERATIONS) {
          ...
          for (const toolCall of brainResponse.toolCalls) {
            ...
            if (!offeredToolNames.has(toolCall.name)) {
              ...
              result = await executeTool(toolCall.name, toolCall.arguments, { tenantId, userId, conversationId });
            }
            request.log.debug(
      ...
      try {
        while (iterationCount < MAX_TOOL_ITERATIONS) {
          ...
          for (const toolCall of brainResponse.toolCalls) {
            ...
            let result = await executeTool(toolCall.name, toolCall.arguments, { tenantId, userId, conversationId });
