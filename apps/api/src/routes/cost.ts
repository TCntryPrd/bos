...
export async function costRoutes(server: FastifyInstance): Promise<void> {
  ...
  server.post('/budget', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    await getPool().query(`INSERT INTO boss_budgets (tenant_id, period, cap_usd, alert_pct, hard_stop) VALUES ($1,$2,$3,$4,$5) ON CONFLICT (tenant_id, period) DO UPDATE SET cap_usd=EXCLUDED.cap_usd, alert_pct=EXCLUDED.alert_pct, hard_stop=EXCLUDED.hard_stop, updated_at=now()`, [tenantId, period, b.cap_usd, b.alert_pct ?? 80, b.hard_stop ?? false]);
