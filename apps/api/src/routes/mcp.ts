...
function canManageMcp(request: FastifyRequest): boolean {
  ...
  return (auth?.authMethod as string | undefined) === 'internal' || auth?.role === 'admin' || auth?.role === 'owner';
}
...
export async function mcpRoutes(server: FastifyInstance): Promise<void> {
  server.get('/global', async (request) => {
    ...
    const hostRegistryPath = firstEnv(['BOSS_GLOBAL_MCP_HOST_REGISTRY_PATH']) ?? '';
    ...
    const authConfigured = Boolean(firstEnv([
    ...
    return {
      id: 'global-mcp',
      name: 'Global MCP',
      scope: 'BOS host / api container project',
      configured,
      ...
      consumers: [
        ...
        },
        {
          ...
          purpose: 'Hermes-owned orchestration runtime and gateway access wired into BOS.',
          canUseGlobalRegistry: true,
        },
      ],
      ...
      agentInstructions: [
        ...
        'The live BOS project runs inside the api container.',
